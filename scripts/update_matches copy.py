...
class TableTennisUpdater:
            ...
            for league_id, league_name in self.leagues.items():
                ...
                try:
                    ...
                    if data.get("success") == 1 and "results" in data:
                        ...
                        for event in events:
                            event_id = str(event.get("id"))  # Garantir que é string
                            total_found += 1
    ...
    def filter_existing_events(self, event_ids):
        ...
        existing_ids = self.get_existing_event_ids()
        new_event_ids = [eid for eid in event_ids if str(eid) not in existing_ids]
    ...
    def save_results_to_db(self, results):
        ...
        for result in results:
            event_id = str(result.get("id"))  # Garantir que é string
    ...
    def update_database(self):
        ...
        event_ids = [str(event["id"]) for event in events]  # Garantir que são strings
